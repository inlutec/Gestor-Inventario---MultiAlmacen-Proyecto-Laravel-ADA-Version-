      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-for="m in movimientos" :key="m.id" class="border-b hover:bg-slate-50" style="position: relative;">
            ...
            <td class="p-2">
              <div v-if="m.almacen" class="text-xs">
                <div class="font-medium">{{ m.almacen.nombre }}</div>
                <div class="text-slate-500">{{ m.almacen.sede }}</div>
              </div>
              <span v-else class="text-slate-400 text-xs">Sin almacén</span>
            </td>
            <td class="p-2">
              <span class="text-xs" :class="m.justificacion.includes('Petición') ? 'text-blue-700 font-medium' : m.justificacion.includes('Pedido') ? 'text-purple-700 font-medium' : 'text-slate-600'">
                {{ m.justificacion }}
              </span>
            </td>
            <td class="p-2">
          ...
          <tr v-if="!movimientos.length">
            <td class="p-3 text-center text-slate-500" colspan="12">Sin resultados</td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function load() {
  ...
  try {
    ...
    // Solo enviar almacen_ids si hay un almacén seleccionado y no es "Todos"
    if (almacenId.value && almacenId.value !== '') {
