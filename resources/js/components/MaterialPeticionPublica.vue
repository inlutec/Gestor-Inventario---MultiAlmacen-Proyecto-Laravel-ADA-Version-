  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-junta-green-50">
    ...
    >
      ...
      <form @submit.prevent="enviarPeticion" class="space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div class="p-4 sm:p-6 space-y-4 sm:space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              ...
              <div>
                <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                  Provincia <span class="text-gray-400">(opcional)</span>
                </label>
                <select
                  ...
                  style="min-height: 48px; font-size: 16px;"
                  @change="cargarSedesPorProvincia"
                >
              ...
              <!-- Selector de Almacén con Mapa -->
              <div class="md:col-span-2">
                <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">
                  Almacén de entrega <span class="text-red-500">*</span>
                </label>
                <AlmacenSelectorMapa
              ...
              <!-- Sede (oculto, se maneja a través del mapa) -->
              <input type="hidden" v-model="form.sede_id" />
              <input type="hidden" v-model="form.departamento_id" />
            </div>
        ...
        <!-- Paso 2: Material Solicitado - Mobile optimized -->
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
          <div class="bg-gradient-to-r from-junta-green-600 to-junta-green-700 px-4 py-3 sm:px-6 sm:py-4">
            <h2 class="text-base sm:text-lg font-bold text-white flex items-center">
        ...
        <!-- Paso 3: Justificación/Observaciones -->
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
          <div class="bg-gradient-to-r from-junta-green-600 to-junta-green-700 px-6 py-4">
            <h2 class="text-lg font-bold text-white flex items-center">
              <span class="bg-white text-junta-green-600 rounded-full w-8 h-8 flex items-center justify-center mr-3 font-bold">3</span>
...
import AlmacenSelectorMapa from '../components/AlmacenSelectorMapa.vue';
...
const onAlmacenChange = (almacen) => {
  almacenSeleccionado.value = almacen;
  // Actualizar sede y departamento automáticamente
  if (almacen?.sede_id) {
    form.value.sede_id = almacen.sede_id;
  }
  if (almacen?.departamento_id) {
    form.value.departamento_id = almacen.departamento_id;
  }
  if (almacen?.provincia_id) {
    form.value.provincia_id = almacen.provincia_id;
  }
};
...
const cargarMateriales = async () => {
  try {
    ...
    if (form.value.provincia_id) {
      params.provincia_id = form.value.provincia_id;
    }
    const response = await axios.get('/materiales-disponibles', { params });
...
const sedeNombre = computed(() => {
  const sede = sedesOriginales.value.find(s => s.id == form.value.sede_id);
  return sede ? sede.nombre : '';
});

const sedesFiltradas = computed(() => {
  if (!form.value.provincia_id) {
    return sedesOriginales.value;
  }
  return sedesOriginales.value.filter(s => s.provincia_id == form.value.provincia_id);
});
...
const cargarSedes = async () => {
  try {
    ...
    if (response.data.success) {
      sedesOriginales.value = response.data.data;
      sedes.value = response.data.data;
    } else {
      sedesOriginales.value = response.data;
      sedes.value = response.data;
    }
    console.log('Sedes cargadas:', sedes.value);
  } catch (err) {
    console.error('Error al cargar sedes:', err);
    sedesOriginales.value = [];
    sedes.value = [];
  }
};

const cargarSedesPorProvincia = async () => {
  // Resetear sede y departamento seleccionados
  form.value.sede_id = '';
  form.value.departamento_id = '';
  departamentos.value = [];
  
  // Las sedes filtradas se actualizan automáticamente por el computed
};
...
const cargarCategorias = async () => {
  try {
    ...
    const response = await axios.get('categorias-publicas', { params });
    if (response.data.success) {
...
const enviarPeticion = async () => {
  ...
  try {
    ...
    if (materialesConStock.length > 0) {
      const payloadPedido = {
        ...form.value,
        almacen_id: almacenSeleccionado.value?.id || null,
