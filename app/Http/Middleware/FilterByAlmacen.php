...
class FilterByAlmacen
{
    ...
    {
        // Para el endpoint de entidades con tipo_entidad_id=3 (materiales), no aplicar filtro
        // ya que los materiales deben ser comunes a todos los almacenes
        if ($request->is('entidades*') && $request->has('tipo_entidad_id') && $request->tipo_entidad_id == 3) {
            return $next($request);
        }
        ...
        if (Auth::check()) {
            ...
            // Solo si el usuario tiene almacenes asignados, para evitar errores
            if (!empty($almacenIds)) {
